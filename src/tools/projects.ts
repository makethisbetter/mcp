...
export function registerProjectShowTool(server: McpServer, client: MakeThisBetterClient): void {
  server.registerTool("project_show", {
    ...
    description: [
      ...
      "Use 'project_list' to find the project id first.",
    ].join(" "),
    inputSchema: {
      id: z.string().min(1).describe("Project prefix ID (e.g. project_xxx)."),
    },
    annotations: {
      readOnlyHint: true,
      openWorldHint: true,
...
export function registerProjectCreateTool(server: McpServer, client: MakeThisBetterClient): void {
  ...
  }, async ({ name, domain }) => runTool(async () => client.createProject({ name, domain })));
}
