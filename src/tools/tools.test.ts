...
describe("shared", () => {
  ...
  describe("hybridToolResult", () => {
    it("splits markdown into text content and keeps the rest as structuredContent", () => {
      const result = hybridToolResult({ id: "fdb_1", status: "received", markdown: "# fdb_1 — received" });
      expect(result).toEqual({
        content: [{ type: "text", text: "# fdb_1 — received" }],
        structuredContent: { id: "fdb_1", status: "received" },
      });
    });

    it("falls back to jsonToolResult when markdown is absent", () => {
      const value = { id: "fdb_1", status: "received" };
      expect(hybridToolResult(value)).toEqual(jsonToolResult(value));
    });

    it("falls back to jsonToolResult when markdown is not a string", () => {
      const value = { id: "fdb_1", markdown: 42 };
      expect(hybridToolResult(value)).toEqual(jsonToolResult(value));
    });
  ...
  describe("runHybridTool", () => {
    it("returns hybrid result on success", async () => {
      const result = await runHybridTool(async () => ({ id: "fdb_1", markdown: "# Hi" }));
      expect(result.content).toEqual([{ type: "text", text: "# Hi" }]);
      expect(result.structuredContent).toEqual({ id: "fdb_1" });
    });
...
describe("list tool", () => {
  ...
  it("returns lean summarized feedbacks", async () => {
    ...
    const result = await handler({ status: "received", limit: 20 });
  ...
  it("passes status and type filters to the client", async () => {
    ...
    await handler({ status: "in_progress", type: "feature", limit: 10 });
...
describe("pick tool", () => {
  it("sets status to in_progress then returns full feedback", async () => {
    const client = mockApiClient();
    client.updateFeedback.mockResolvedValue({ id: "fdb_1", status: "in_progress" });
    ...
    const result = await handler({ feedback_id: "fdb_1" });
    expect(client.updateFeedback).toHaveBeenCalledWith("fdb_1", { status: "in_progress" });
    expect(client.getFeedback).toHaveBeenCalledWith("fdb_1");

    const parsed = parseResult(result) as Record<string, unknown>;
    expect(parsed).toMatchObject({ id: "fdb_1", status: "in_progress", description: "Full context" });
  ...
  it("returns markdown text and structuredContent when the API includes markdown", async () => {
    ...
    client.getFeedback.mockResolvedValue({
      id: "fdb_1",
      status: "in_progress",
      markdown: "# fdb_1 — In progress",
    });
    const handler = setup(registerPickTool, client);

    const result = await handler({ feedback_id: "fdb_1" });
    expect(result.content).toEqual([{ type: "text", text: "# fdb_1 — In progress" }]);
    expect(result.structuredContent).toEqual({ id: "fdb_1", status: "in_progress" });
  });
  ...
  it("calls updateFeedback before getFeedback", async () => {
    ...
    client.getFeedback.mockImplementation(async () => { order.push("get"); return { id: "fdb_1" }; });
    const handler = setup(registerPickTool, client);

    await handler({ feedback_id: "fdb_1" });
...
describe("dismiss tool", () => {
  it("closes with not_planned by default, nested in labels", async () => {
    const client = mockApiClient();
    client.updateFeedback.mockResolvedValue({ id: "fdb_1", status: "closed", close_reason: "not_planned" });
    const handler = setup(registerDismissTool, client);

    const result = await handler({ feedback_id: "fdb_1" });
    expect(client.updateFeedback).toHaveBeenCalledWith("fdb_1", {
      status: "closed",
      labels: { close_reason: "not_planned" },
    });
    expect(parseResult(result)).toMatchObject({ id: "fdb_1", close_reason: "not_planned" });
  });

  it("closes as duplicate when close_reason is provided", async () => {
    const client = mockApiClient();
    client.updateFeedback.mockResolvedValue({ id: "fdb_1", status: "closed", close_reason: "duplicate" });
    const handler = setup(registerDismissTool, client);

    await handler({ feedback_id: "fdb_1", close_reason: "duplicate" });
    expect(client.updateFeedback).toHaveBeenCalledWith("fdb_1", {
      status: "closed",
...
describe("resolve tool", () => {
  it("closes with shipped reason nested in labels", async () => {
    const client = mockApiClient();
    client.updateFeedback.mockResolvedValue({ id: "fdb_1", status: "closed", close_reason: "shipped" });
    ...
    expect(parseResult(result)).toMatchObject({ id: "fdb_1", close_reason: "shipped" });
  });
  ...
  it("includes pr_url in labels when provided", async () => {
    ...
    await handler({ feedback_id: "fdb_1", pr_url: "https://github.com/acme/app/pull/42" });
    expect(client.updateFeedback).toHaveBeenCalledWith("fdb_1", {
      status: "closed",
...
describe("detail tool", () => {
  ...
  it("does not call updateFeedback (read-only)", async () => {
    const client = mockApiClient();
    client.getFeedback.mockResolvedValue({ id: "fdb_1" });
    const handler = setup(registerDetailTool, client);
  ...
  it("returns error on API failure", async () => {
    ...
    const result = await handler({ feedback_id: "fdb_missing" });
    expect(result.isError).toBe(true);
...
describe("project_list tool", () => {
  it("returns the projects array as-is", async () => {
    const client = mockApiClient();
    const projects = [
      { id: "project_1", name: "Acme", domain: null, feedback_visibility: "public", created_at: "2026-01-01", updated_at: "2026-01-02", feedbacks_count: 3 },
    ];
...
describe("project_show tool", () => {
  it("returns full project detail including api_key and board_url", async () => {
    ...
    const result = await handler({ id: "project_1" });
    expect(client.getProject).toHaveBeenCalledWith("project_1");
    expect(parseResult(result)).toEqual(project);
  });

  it("includes signing_secret when present for account admins", async () => {
    const client = mockApiClient();
    client.getProject.mockResolvedValue({ id: "project_1", signing_secret: "whsec_abc" });
    const handler = setup(registerProjectShowTool, client);
  ...
  it("omits signing_secret when absent (non-admin)", async () => {
    const client = mockApiClient();
    client.getProject.mockResolvedValue({ id: "project_1", api_key: "mtb_proj_abc" });
    const handler = setup(registerProjectShowTool, client);
  ...
  it("returns error on API failure", async () => {
    ...
    const result = await handler({ id: "project_missing" });
    expect(result.isError).toBe(true);
    expect(result.content[0]).toMatchObject({ type: "text", text: "not found" });
  });
});
...
describe("project_create tool", () => {
  it("creates a project with name and domain", async () => {
    const client = mockApiClient();
    client.createProject.mockResolvedValue({ id: "project_1", name: "New", domain: "example.com", api_key: "mtb_proj_new" });
    const handler = setup(registerProjectCreateTool, client);

    const result = await handler({ name: "New", domain: "example.com" });
    expect(client.createProject).toHaveBeenCalledWith({ name: "New", domain: "example.com" });
    expect(parseResult(result)).toMatchObject({ id: "project_1", name: "New" });
  });

  it("creates a project without a domain", async () => {
    const client = mockApiClient();
    client.createProject.mockResolvedValue({ id: "project_1", name: "New" });
    const handler = setup(registerProjectCreateTool, client);

    await handler({ name: "New" });
    expect(client.createProject).toHaveBeenCalledWith({ name: "New", domain: undefined });
  });
  ...
  it("returns error when not an account admin", async () => {
    ...
    const result = await handler({ name: "New" });
    expect(result.isError).toBe(true);
